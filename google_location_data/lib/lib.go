...
func (sourceData *SourceLocationData) FindClosestLocation(time time.Time) (ind int, err error) {
	var (
	...
	afterTime := sourceData.Locations[locationAfterInd].GetTime()
	beforeTime := sourceData.Locations[locationBeforeInd].GetTime()
